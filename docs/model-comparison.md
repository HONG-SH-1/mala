...
| 1차 운영 모델 | **채택 예정:** Qwen3-8B-Instruct Q4 (또는 실측 최적) |
...
|-------------|-------|----------------|------------------|-----------|-------------|----------------|------|
| Qwen3-8B-Instruct | Q4_K_M | 8B dense | ~4.5–5.5 GB | ✅ | 단일 파이프라인 | _TBD_ | **Phase 1 기본 후보** |
...
1. [ ] CPU / RAM / OS — `hardware-environment.md` §3
2. [ ] 드라이버·CUDA 버전 기록
2. [ ] 모델 ID·quant·context 길이 고정
3. [ ] cold load 후 `nvidia-smi` peak VRAM 스크린샷
